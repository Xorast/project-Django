...
class TestActivitiesViews(TestCase):
    ...
    def test_animation_details_page(self):
        page = self.client.get(reverse("a_animation_details", args=[self.activity_animation_type.animation_type, self.activity_animation.name, self.activity_animation.id ]))
        self.assertEqual(page.status_code, 200)
    
    def test_activity_registration_page(self):
        login = self.client.login(username='testuser', password='12345')
        page = self.client.get(reverse("a_animation_slot_registration", args=[self.activity_animation_type.animation_type, self.activity_animation.name, self.activity_animation.id, self.activity_animation_slot.id ]))
        self.assertEqual(page.status_code, 200)
